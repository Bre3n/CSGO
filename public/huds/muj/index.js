...
const killfeedEntry = (kill) => {
  const assistHTML = kill.assister
    ...
    <div class="assist-container">
     ...
     <div class="assist-name ${kill.assister.team.side}">${
        kill.assister.name
      }</div>
   </div>
   `
...
const fillAvatar = (side, number, steamid) => {
  ...
  playerAvatars[steamid].then((avatar) => {
    ...
    if (
      $(`#players_${side} #player${number} #player_image`).attr("src") ===
      (avatar.custom || avatar.steam)
    )
      return;
    $(`#players_${side} #player${number} #player_image`).attr(
      "src",
      avatar.custom || avatar.steam
    );
  });
  ...
  if (phase) {
    ...
    if (bomb != null) {
      if (bomb.state == "planting") {
        ...
        // 13 characters for name
        showAlertSlide(side, COLOR_NEW_T, planter.name + " podkłada bombę");
      }
    }
  }
}
    ...
    $("#alert_middle #alert_text_middle")
      .text(
        teams.left.side == "t"
          ...
          : " PRZERWA: " + teams.right.name.toUpperCase()
      )
      .css("color", COLOR_NEW_T);
    showAlertSlide(
      "#left_team",
